<template>
  <article class="max-w-4xl mx-auto px-4 py-8">
    ...
    <div class="prose prose-lg dark:prose-invert max-w-none">
      ...
      <p>
        生成AIへの指示は<strong>「制約を与える」</strong>行為です。ここでいう制約とは、単に「できないこと」を増やすのではなく、「この方向に進んでほしい」という道筋を示すこと。生成AIは、生成できる無数の可能性から、与えられた制約に最も適合するものを選び出して返します。
      </p>
      ...
      <p>
        制約が少なすぎると、AIの解釈が入り込み、結果が発散します。たとえば「わかりやすく書いて」だけでは、誰にとってわかりやすいのか、何を重視するのかが読み取れず、10回生成すれば10通りの異なる結果が返ってくる。
      </p>
      <p>
        一方、制約が多すぎると、指示を書くコストが高くなり、AIを使う意味が薄れます。「見出しは18pt、本文は12pt、行間は1.5倍...」と細かく指定するなら、自分で作った方が早い。
      </p>
      <p>
        さらに言えば、2025年末の時点で、生成AIが個別具体的なタスクをこなす能力はすでにほとんどの人間を超えています。つまり、制約を過剰に増やすよりも、AIに任せた方が品質が高まるケースが多いということになります。
      </p>
      ...
      <p>
        <strong>幹</strong
        >とは、<strong>「制約と品質のスイートスポット」</strong>のこと。これがあれば、残りの判断は生成AIが導出できます。言い換えれば、品質を維持できる最小限の情報です。
      </p>
      <p class="text-sm text-gray-600 dark:text-gray-400">
        例えば、「決裁者向け資料」という幹があれば、「専門用語は避ける」「結論を先に」「ページ数は最小限に」といった判断は自然と決まります。これらをひとつひとつ指定する必要はありません。
      </p>
      <p class="text-sm text-gray-600 dark:text-gray-400">
        なお、「種・幹・枝葉」の境界は文脈で変わります。「メールに署名を入れる」は多くの場合は枝葉ですが、社名変更を伝えるメールでは幹になりえる。判断基準は「それが決まると、他の判断が連鎖的に決まるか」です。
      </p>
      ...
      <p>
        後者では読み手・目的・必要情報といった制約を渡していますが、構成や表現は指定していない。これが「幹を渡す」ということです。
      </p>
      ...
      <Alert type="danger" title="パターン2: 目的が曖昧">
        <p class="mb-2">
      ...
      <StepFlow>
        <Step number="1" title="幹を伝えて骨子を出力させる">
          詳細は求めず、幹となる制約を渡して成果物のアウトラインを出力させる。方向性がきちんと伝わっているかを確認します。
        </Step>
        <Step number="2" title="違和感を伝えてコンテキストを育てる">
          アウトラインに違和感があれば、それを言語化して伝える。この往復でAI側にも文脈が蓄積され、認識が揃っていきます。
        </Step>
        <Step number="3" title="枝葉の詳細化を委譲する">
          方向が定まれば、表現や構成の細部はAIに任せる。もし微調整が必要なら、直接編集してもかまいません。（しかし、多くを書き換えているようなら、幹が不十分でしょう）
        </Step>
      </StepFlow>

      <p>
        対話を重ねるごとにAIとのコンテキストが育ち、指示は短く、出力の精度は高くなります。幹を見つけ、渡すことは、一度きりの完璧な指示ではなく、AIとの対話を通じた協働作業です。
      </p>
      ...
      <p>
        今「幹」として渡している情報も、いずれ不要になるかもしれません。AIがコードベースや設計ドキュメント全体を自律的に理解し、コンテキストから幹を推測できるようになれば、人間が明示的に渡す情報はさらに減る。毎朝、AIから次に開発すべき機能が提案される日も近いかもしれません。
      </p>
      <p>
      ...
      <p>
        AIが進化しても、この力の重要性は変わりません。幹を見つける、渡す、蓄積する、見極める——フェーズは変わっても、幹を理解している人間が価値を生み出す構造は同じです。
      </p>
